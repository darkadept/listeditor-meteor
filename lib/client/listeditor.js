...
function saveItem(ev, tmpl) {
	...
	// Add tags to their collection
	tmpl.$('form .widget-select2.tags').not('.select2-container').each(function(){
		var $this = $(this);

		var collection = getCollection($this.data('collection'));
		var key = $this.data('key');
		var name = $this.attr('name');

		if (collection && key && name) {
			var tags = $this.select2('data');
...
function setDataOnMediumEditor(data) {
	$('.widget-mediumeditor').each(function(){
		var $this = $(this);
...
function setDataOnTypeahead(data) {
	$('.widget-typeahead').each(function(){
		var $this = $(this);
		var name = $this.attr('name');
		if (data && data[name]) {
